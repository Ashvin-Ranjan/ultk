...
if __name__ == "__main__":
    ...
    dominating_languages = yaml_to_dataframe(
        "family/outputs/dominating_languages.yml", keys
    )
    explored_languages = yaml_to_dataframe(
        "family/outputs/explored_languages.yml", keys
    )
    natural_languages = yaml_to_dataframe(
        "family/outputs/natural_languages.yml", keys
    )
    all_data = pd.concat(
        [explored_languages, dominating_languages, natural_languages], ignore_index=True
    )
    all_data.to_csv("family/outputs/combined_data.csv", index=False)
