...
if __name__ == "__main__":
    expressions_by_meaning: dict[Meaning, GrammaticalExpression] = (
        family_grammar.get_unique_expressions(
            6,
            max_size=2 ** len(family_universe),
            unique_key=lambda expr: expr.evaluate(family_universe),
    ...
    write_expressions(
        expressions_by_meaning.values(), "family/outputs/generated_expressions.yml"
    )
